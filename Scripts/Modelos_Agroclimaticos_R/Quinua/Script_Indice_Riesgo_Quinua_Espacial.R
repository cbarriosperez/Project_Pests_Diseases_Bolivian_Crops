## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - QUINUA / TICONA
...
## para un solo punto (fila 17: Quinua / Ticona).
##
## Umbral de alerta (columna P17 del Excel):
##   ALERTA si: T media 15-25 C  y  precipitacion >= 30 mm  y  >= 3 dias nublados/lluviosos.
##
## Indicador propuesto (columna R17):
##   Indice templado-lluvioso = coincidencia de T media 15-25 C, precipitacion >= 30 mm y >= 3 dias nublados/lluviosos.
##
## La formula de esa hoja (celda AC17) suma, con sus pesos:
##     (T media entre 15 y 25 C) * 1
##     (precipitacion acumulada >= 30 mm) * 2
##     (dias lluvia/nublados >= 3) * 1
##   ... y se divide entre la suma de los pesos (4).
##
## y el semaforo de alerta (misma logica que la celda S17) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de quinua, o sin dato climatico,
...
## ----------------------------
## La columna I17 define la precipitacion favorable como "≥ 30 mm/10 dias"; se
## usa un periodo de 10 dias para igualar esa ventana.
...
# --- 1.3 Shapefiles: region productora de quinua y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/QUINUA/QUINUA.shp"
...
# unicamente dentro de la region de quinua).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_quinua_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC17 del
##     Excel) para Quinua / Ticona.
## ----------------------------------------------------------------------
calcular_indice_quinua <- function(t_media_periodo, precip_acum_periodo, dias_lluvia_periodo) {

  cond_t <- t_media_periodo >= 15 & t_media_periodo <= 25   # T media entre 15 y 25 C
  cond_precip <- precip_acum_periodo >= 30   # precipitacion acumulada >= 30 mm
  cond_lluvia <- dias_lluvia_periodo >= 3   # dias lluvia/nublados >= 3

  # pesos: 1, 2, 1 (tal cual columnas Z17:AB17 del Excel)
  (cond_t * 1 + cond_precip * 2 + cond_lluvia * 1) / 4
}
...
# recortar y enmascarar todo a la region productora de quinua
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_quinua(t_media_periodo, precip_acum_periodo, dias_lluvia_periodo)
  ...
  cond_t_r <- t_media_periodo >= 15 & t_media_periodo <= 25
  cond_precip_r <- precip_acum_periodo >= 30
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Ticona en Quinua",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 15-25C, precip. >=30mm y >=3 dias lluvia/nublados") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_quinua_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_quinua_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_quinua_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_quinua_2025.csv"),
          row.names = FALSE)

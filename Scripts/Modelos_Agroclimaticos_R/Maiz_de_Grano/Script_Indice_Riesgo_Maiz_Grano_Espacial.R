## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - MAIZ DE GRANO / ROYA
...
## para un solo punto (fila 14: Maiz de grano / Roya).
##
## Umbral de alerta (columna P14 del Excel):
##   ALERTA si: T media 15-28 C  y  HR >= 75%  y  precipitacion >= 30 mm.
##
## Indicador propuesto (columna R14):
##   Indice de humedad foliar = coincidencia de T media 15-28 C, HR >= 75% y precipitacion >= 30 mm.
##
## La formula de esa hoja (celda AC14) suma, con sus pesos:
##     (T media entre 15 y 28 C) * 1
##     (HR media >= 75%) * 2
##     (precipitacion acumulada >= 30 mm) * 1
##   ... y se divide entre la suma de los pesos (4).
##
## y el semaforo de alerta (misma logica que la celda S14) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de maiz de grano, o sin dato climatico,
...
## ----------------------------
## La columna I14 define la precipitacion favorable como "≥ 30 mm/10 dias"; se
...
# --- 1.3 Shapefiles: region productora de maiz de grano y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/MAIZ_EN_GRANO/MAIZ_EN_GRANO.shp"
...
# unicamente dentro de la region de maiz de grano).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_maiz_grano_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC14 del
##     Excel) para Maiz de grano / Roya.
## ----------------------------------------------------------------------
calcular_indice_maiz_grano <- function(t_media_periodo, hr_media_periodo, precip_acum_periodo) {

  cond_t <- t_media_periodo >= 15 & t_media_periodo <= 28   # T media entre 15 y 28 C
  cond_hr <- hr_media_periodo >= 75   # HR media >= 75%
  cond_precip <- precip_acum_periodo >= 30   # precipitacion acumulada >= 30 mm

  # pesos: 1, 2, 1 (tal cual columnas Z14:AB14 del Excel)
...
# recortar y enmascarar todo a la region productora de maiz de grano
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_maiz_grano(t_media_periodo, hr_media_periodo, precip_acum_periodo)
  ...
  cond_t_r <- t_media_periodo >= 15 & t_media_periodo <= 28
  cond_hr_r <- hr_media_periodo >= 75
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Roya en Maiz de grano",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 15-28C, HR >= 75% y precip. >= 30 mm") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_maiz_grano_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_maiz_grano_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_maiz_grano_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_maiz_grano_2025.csv"),
          row.names = FALSE)

## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - DURAZNO / OIDIO
...
## para un solo punto (fila 10: Durazno / Oidio).
##
## Umbral de alerta (columna P10 del Excel):
##   ALERTA si: T media 18-25 C  y  HR 50-70%.
##
## Indicador propuesto (columna R10):
##   Indice templado-humedo moderado = coincidencia de T media 18-25 C y HR 50-70% (favorable al oidio).
##
## La formula de esa hoja (celda AC10) suma, con sus pesos:
##     (T media entre 18 y 25 C) * 1
##     (HR media entre 50 y 70%) * 1
##   ... y se divide entre la suma de los pesos (2).
##
## y el semaforo de alerta (misma logica que la celda S10) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de durazno, o sin dato climatico,
...
## ----------------------------
## El Excel no define una ventana de dias explicita para esta plaga (solo rangos
## de T y HR). Se usa el mismo periodo de 10 dias que Aranuela roja (fila 9), por
## tratarse del mismo cultivo y region, para poder comparar ambos modelos con la
## misma periodizacion.
...
# --- 1.3 Shapefiles: region productora de durazno y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/DURAZNO/DURAZNO.shp"
...
# unicamente dentro de la region de durazno).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_durazno_oidio_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC10 del
##     Excel) para Durazno / Oidio.
## ----------------------------------------------------------------------
calcular_indice_durazno_oidio <- function(t_media_periodo, hr_media_periodo) {

  cond_t <- t_media_periodo >= 18 & t_media_periodo <= 25   # T media entre 18 y 25 C
  cond_hr <- hr_media_periodo >= 50 & hr_media_periodo <= 70   # HR media entre 50 y 70%

  # pesos: 1, 1 (tal cual columnas Z10:AB10 del Excel)
  (cond_t * 1 + cond_hr * 1) / 2
}
...
# recortar y enmascarar todo a la region productora de durazno
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_durazno_oidio(t_media_periodo, hr_media_periodo)
  ...
  cond_t_r <- t_media_periodo >= 18 & t_media_periodo <= 25
  cond_hr_r <- hr_media_periodo >= 50 & hr_media_periodo <= 70
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Oidio en Durazno",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 18-25C y HR entre 50 y 70%") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_durazno_oidio_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_durazno_oidio_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_durazno_oidio_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_durazno_oidio_2025.csv"),
          row.names = FALSE)

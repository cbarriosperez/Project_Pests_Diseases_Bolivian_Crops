## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - DURAZNO / ARANUELA ROJA
...
## para un solo punto (fila 9: Durazno / Aranuela roja).
##
## Umbral de alerta (columna P9 del Excel):
##   ALERTA si: T media > 25 C  y  >= 7 dias secos consecutivos  y  HR < 50%.
##
## Indicador propuesto (columna R9):
##   Indice seco-calido = coincidencia de T media > 25 C, >= 7 dias secos consecutivos y HR < 50% (favorable a aranuela roja).
##
## La formula de esa hoja (celda AC9) suma, con sus pesos:
##     (T media > 25 C) * 2
##     (>= 7 dias secos consecutivos) * 1
##     (HR media < 50%) * 1
##   ... y se divide entre la suma de los pesos (4).
##
## y el semaforo de alerta (misma logica que la celda S9) es:
##   indice = 0    -> "Normal"
...
## ----------------------------
## La columna K9 exige "≥ 7 dias secos consecutivos"; se usa un periodo de 10
## dias (con la misma logica que Ajo) para dar holgura suficiente a que se
## observe esa racha dentro de la ventana.
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_durazno_aranuela_2025"
...
## ----------------------------------------------------------------------
## 3.3 Formula EXACTA del indice de riesgo (replica celda AC9 del
##     Excel) para Durazno / Aranuela roja.
## ----------------------------------------------------------------------
calcular_indice_durazno_aranuela <- function(t_media_periodo, dias_secos_periodo, hr_media_periodo) {

  cond_t <- t_media_periodo > 25   # T media > 25 C
  cond_secos <- dias_secos_periodo >= 7   # >= 7 dias secos consecutivos
  cond_hr <- hr_media_periodo < 50   # HR media < 50%

  # pesos: 2, 1, 1 (tal cual columnas Z9:AB9 del Excel)
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_durazno_aranuela(t_media_periodo, dias_secos_periodo, hr_media_periodo)
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Aranuela roja en Durazno",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media >25C, >=7 dias secos consecutivos y HR <50%") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_durazno_aranuela_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_durazno_aranuela_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_durazno_aranuela_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_durazno_aranuela_2025.csv"),
          row.names = FALSE)

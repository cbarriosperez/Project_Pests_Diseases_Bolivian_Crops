## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - MAIZ CHOCLO / GUSANO COGOLLERO
...
## para un solo punto (fila 13: Maiz choclo / Gusano cogollero).
##
## Umbral de alerta (columna P13 del Excel):
##   ALERTA si: T media >= 22 C  y  precipitacion >= 20 mm.
##
## Indicador propuesto (columna R13):
##   Indice calido-lluvioso = coincidencia de T media >= 22 C y precipitacion >= 20 mm (inicio de lluvias).
##
## La formula de esa hoja (celda AC13) suma, con sus pesos:
##     (T media >= 22 C) * 2
##     (precipitacion acumulada >= 20 mm) * 1
##   ... y se divide entre la suma de los pesos (3).
##
## y el semaforo de alerta (misma logica que la celda S13) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de maiz choclo, o sin dato climatico,
...
## ----------------------------
## La columna I13 define la precipitacion favorable como "inicio de lluvias (≥ 20
## mm/semana)"; se usa un periodo de 7 dias (semanal) para igualar esa ventana.
...
# --- 1.3 Shapefiles: region productora de maiz choclo y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/MAIZ_CHOCLO/MAIZ_CHOCLO.shp"
...
# unicamente dentro de la region de maiz choclo).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_maiz_choclo_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC13 del
##     Excel) para Maiz choclo / Gusano cogollero.
## ----------------------------------------------------------------------
calcular_indice_maiz_choclo <- function(t_media_periodo, precip_acum_periodo) {

  cond_t <- t_media_periodo >= 22   # T media >= 22 C
  cond_precip <- precip_acum_periodo >= 20   # precipitacion acumulada >= 20 mm

  # pesos: 2, 1 (tal cual columnas Z13:AB13 del Excel)
...
# recortar y enmascarar todo a la region productora de maiz choclo
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_maiz_choclo(t_media_periodo, precip_acum_periodo)
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Gusano cogollero en Maiz choclo",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media >=22C y precip. >= 20 mm") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_maiz_choclo_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_maiz_choclo_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_maiz_choclo_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_maiz_choclo_2025.csv"),
          row.names = FALSE)

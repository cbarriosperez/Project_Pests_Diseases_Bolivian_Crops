## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - FRUTILLA / BOTRYTIS (MOHO GRIS)
...
## para un solo punto (fila 11: Frutilla / Botrytis (moho gris)).
##
## Umbral de alerta (columna P11 del Excel):
##   ALERTA si: T media 15-23 C  y  HR >= 90%  y  >= 2 dias nublados/lluviosos.
##
## Indicador propuesto (columna R11):
##   Indice de humedad en floracion = coincidencia de T media 15-23 C, HR >= 90% y >= 2 dias nublados/lluviosos.
##
## La formula de esa hoja (celda AC11) suma, con sus pesos:
##     (T media entre 15 y 23 C) * 1
##     (HR media >= 90%) * 2
##     (dias lluvia/nublados >= 2) * 1
##   ... y se divide entre la suma de los pesos (4).
##
## y el semaforo de alerta (misma logica que la celda S11) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de frutilla, o sin dato climatico,
...
## ----------------------------
## La columna I11 define la precipitacion favorable como "≥ 40 mm/10 dias"; se
## usa un periodo de 10 dias para mantener esa misma ventana de referencia.
...
# --- 1.3 Shapefiles: region productora de frutilla y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/FRUTILLA/FRUTILLA.shp"
...
# unicamente dentro de la region de frutilla).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_frutilla_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC11 del
##     Excel) para Frutilla / Botrytis (moho gris).
## ----------------------------------------------------------------------
calcular_indice_frutilla <- function(t_media_periodo, hr_media_periodo, dias_lluvia_periodo) {

  cond_t <- t_media_periodo >= 15 & t_media_periodo <= 23   # T media entre 15 y 23 C
  cond_hr <- hr_media_periodo >= 90   # HR media >= 90%
  cond_lluvia <- dias_lluvia_periodo >= 2   # dias lluvia/nublados >= 2

  # pesos: 1, 2, 1 (tal cual columnas Z11:AB11 del Excel)
...
# recortar y enmascarar todo a la region productora de frutilla
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_frutilla(t_media_periodo, hr_media_periodo, dias_lluvia_periodo)
  ...
  cond_t_r <- t_media_periodo >= 15 & t_media_periodo <= 23
  cond_hr_r <- hr_media_periodo >= 90
  cond_lluvia_r <- dias_lluvia_periodo >= 2
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Botrytis (moho gris) en Frutilla",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 15-23C, HR >= 90% y >=2 dias lluvia/nublados") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_frutilla_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_frutilla_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_frutilla_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_frutilla_2025.csv"),
          row.names = FALSE)

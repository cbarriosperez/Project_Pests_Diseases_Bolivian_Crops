## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - CAFE / BROCA DEL CAFE
...
## para un solo punto (fila 7: Cafe / Broca del Cafe).
##
## Umbral de alerta (columna P7 del Excel):
##   ALERTA si: T media 20-30 C  y  precipitacion 30-80 mm.
##
## Indicador propuesto (columna R7):
##   Indice calido-humedo = coincidencia de T media 20-30 C y precipitacion moderada (30-80 mm).
##
## La formula de esa hoja (celda AC7) suma, con sus pesos:
##     (T media entre 20 y 30 C) * 2
##     (precipitacion acumulada entre 30 y 80 mm) * 1
##   ... y se divide entre la suma de los pesos (3).
##
## y el semaforo de alerta (misma logica que la celda S7) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de cafe, o sin dato climatico,
...
##
## ============================================================================
## POR QUE PERIODOS DE 30 DIAS
## ----------------------------
## La columna I7 del Excel define la precipitacion favorable como "30-80 mm/mes";
## por eso se usa un periodo de 30 dias (aproximando el mes calendario) para que
## la acumulacion de precipitacion sea comparable con esa referencia
## bibliografica.
## Por eso PERIODO_DIAS = 30 es el valor recomendado por defecto.
...
# --- 1.1 Periodizacion --------------------------------------------------
periodo_dias   <- 30                       # largo de cada periodo (dias). Recomendado: 30
...
# --- 1.3 Shapefiles: region productora de cafe y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/CAFE/CAFE.shp"
...
# unicamente dentro de la region de cafe).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_cafe_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC7 del
##     Excel) para Cafe / Broca del Cafe.
## ----------------------------------------------------------------------
calcular_indice_cafe <- function(t_media_periodo, precip_acum_periodo) {

  cond_t <- t_media_periodo >= 20 & t_media_periodo <= 30   # T media entre 20 y 30 C
  cond_precip <- precip_acum_periodo >= 30 & precip_acum_periodo <= 80   # precipitacion acumulada entre 30 y 80 mm

  # pesos: 2, 1 (tal cual columnas Z7:AB7 del Excel)
  (cond_t * 2 + cond_precip * 1) / 3
}
...
# recortar y enmascarar todo a la region productora de cafe
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_cafe(t_media_periodo, precip_acum_periodo)
  ...
  cond_t_r <- t_media_periodo >= 20 & t_media_periodo <= 30
  cond_precip_r <- precip_acum_periodo >= 30 & precip_acum_periodo <= 80
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Broca del Cafe en Cafe",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 20-30C y precip. entre 30 y 80 mm") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_cafe_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_cafe_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_cafe_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_cafe_2025.csv"),
          row.names = FALSE)

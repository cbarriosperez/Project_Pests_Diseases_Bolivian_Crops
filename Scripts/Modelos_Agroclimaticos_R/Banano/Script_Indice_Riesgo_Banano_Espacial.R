## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - BANANO / SIGATOKA NEGRA
...
## para un solo punto (fila 6: Banano / Sigatoka negra).
##
## Umbral de alerta (columna P6 del Excel):
##   ALERTA si: T media 25-28 C  y  HR >= 90%  y  precipitacion >= 50 mm.
##
## Indicador propuesto (columna R6):
##   Indice de humedad foliar = coincidencia de T media 25-28 C, HR >= 90% y precipitacion >= 50 mm (favorable a esporulacion).
##
## La formula de esa hoja (celda AC6) suma, con sus pesos:
##     (T media entre 25 y 28 C) * 1
##     (HR media >= 90%) * 2
##     (precipitacion acumulada >= 50 mm) * 1
##   ... y se divide entre la suma de los pesos (4).
##
## y el semaforo de alerta (misma logica que la celda S6) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de banano, o sin dato climatico,
...
## ----------------------------
## La columna I6 del Excel define la precipitacion favorable como "≥ 50 mm / 10
## dias", por eso se usa un periodo de 10 dias: es la misma ventana en la que
## esta documentada la humedad foliar prolongada que necesita el hongo.
...
# --- 1.3 Shapefiles: region productora de banano y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/BANANO/BANANO.shp"
...
# unicamente dentro de la region de banano).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_banano_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC6 del
##     Excel) para Banano / Sigatoka negra.
## ----------------------------------------------------------------------
calcular_indice_banano <- function(t_media_periodo, hr_media_periodo, precip_acum_periodo) {

  cond_t <- t_media_periodo >= 25 & t_media_periodo <= 28   # T media entre 25 y 28 C
  cond_hr <- hr_media_periodo >= 90   # HR media >= 90%
  cond_precip <- precip_acum_periodo >= 50   # precipitacion acumulada >= 50 mm

  # pesos: 1, 2, 1 (tal cual columnas Z6:AB6 del Excel)
  (cond_t * 1 + cond_hr * 2 + cond_precip * 1) / 4
}
...
# recortar y enmascarar todo a la region productora de banano
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_banano(t_media_periodo, hr_media_periodo, precip_acum_periodo)
  ...
  cond_t_r <- t_media_periodo >= 25 & t_media_periodo <= 28
  cond_hr_r <- hr_media_periodo >= 90
  cond_precip_r <- precip_acum_periodo >= 50
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Sigatoka negra en Banano",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 25-28C, HR >= 90% y precip. >= 50 mm") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_banano_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_banano_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_banano_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_banano_2025.csv"),
          row.names = FALSE)

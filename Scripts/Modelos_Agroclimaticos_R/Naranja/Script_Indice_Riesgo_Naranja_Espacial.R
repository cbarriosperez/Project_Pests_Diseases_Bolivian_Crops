## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - NARANJA / MOSCA DE LA FRUTA
...
## para un solo punto (fila 15: Naranja / Mosca de la fruta).
##
## Umbral de alerta (columna P15 del Excel):
##   ALERTA si: T media 20-30 C  y  >= 10 dias secos consecutivos.
##
## Indicador propuesto (columna R15):
##   Indice calido-seco = coincidencia de T media 20-30 C y >= 10 dias secos consecutivos.
##
## La formula de esa hoja (celda AC15) suma, con sus pesos:
##     (T media entre 20 y 30 C) * 1
##     (>= 10 dias secos consecutivos) * 2
##   ... y se divide entre la suma de los pesos (3).
##
## y el semaforo de alerta (misma logica que la celda S15) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de naranja, o sin dato climatico,
...
## ----------------------------
## La columna K15 exige "≥ 10 dias secos consecutivos" (la misma referencia que
## Ajo); se usa igualmente un periodo de 15 dias para dar holgura suficiente a
## esa racha dentro de la ventana.
...
# --- 1.3 Shapefiles: region productora de naranja y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/NARANJA/NARANJA.shp"
...
# unicamente dentro de la region de naranja).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_naranja_2025"
...
## ----------------------------------------------------------------------
## 3.3 Formula EXACTA del indice de riesgo (replica celda AC15 del
##     Excel) para Naranja / Mosca de la fruta.
## ----------------------------------------------------------------------
calcular_indice_naranja <- function(t_media_periodo, dias_secos_periodo) {
  ...
  # pesos: 1, 2 (tal cual columnas Z15:AB15 del Excel)
  (cond_t * 1 + cond_secos * 2) / 3
}
...
# recortar y enmascarar todo a la region productora de naranja
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_naranja(t_media_periodo, dias_secos_periodo)
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Mosca de la fruta en Naranja",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 20-30C y >=10 dias secos consecutivos") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_naranja_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_naranja_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_naranja_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_naranja_2025.csv"),
          row.names = FALSE)

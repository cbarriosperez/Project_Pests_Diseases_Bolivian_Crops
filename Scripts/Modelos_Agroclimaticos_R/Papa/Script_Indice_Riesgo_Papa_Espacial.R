## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - PAPA / GORGOJO DE LOS ANDES
...
## para un solo punto (fila 16: Papa / Gorgojo de los Andes).
##
## Umbral de alerta (columna P16 del Excel):
##   ALERTA si: T media <= 15 C  y  precipitacion >= 30 mm.
##
## Indicador propuesto (columna R16):
##   Indice frio-humedo = coincidencia de T media <= 15 C y precipitacion >= 30 mm (inicio de lluvias).
##
## La formula de esa hoja (celda AC16) suma, con sus pesos:
##     (T media <= 15 C) * 1
##     (precipitacion acumulada >= 30 mm) * 2
##   ... y se divide entre la suma de los pesos (3).
##
## y el semaforo de alerta (misma logica que la celda S16) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de papa, o sin dato climatico,
...
## ----------------------------
## El Excel no define una ventana de dias explicita para esta plaga (solo "inicio
## de lluvias, ≥ 30 mm"); se usa el periodo dekadal estandar de 10 dias, habitual
## en el monitoreo agroclimatico.
...
# --- 1.3 Shapefiles: region productora de papa y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/PAPA/PAPA.shp"
...
# unicamente dentro de la region de papa).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_papa_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC16 del
##     Excel) para Papa / Gorgojo de los Andes.
## ----------------------------------------------------------------------
calcular_indice_papa <- function(t_media_periodo, precip_acum_periodo) {

  cond_t <- t_media_periodo <= 15   # T media <= 15 C
  cond_precip <- precip_acum_periodo >= 30   # precipitacion acumulada >= 30 mm

  # pesos: 1, 2 (tal cual columnas Z16:AB16 del Excel)
  (cond_t * 1 + cond_precip * 2) / 3
}
...
# recortar y enmascarar todo a la region productora de papa
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_papa(t_media_periodo, precip_acum_periodo)
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Gorgojo de los Andes en Papa",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media <=15C y precip. >= 30 mm") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_papa_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_papa_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_papa_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_papa_2025.csv"),
          row.names = FALSE)

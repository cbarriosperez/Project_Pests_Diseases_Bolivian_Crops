## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - CEBOLLA / MILDIU (POLVILLO NEGRO)
...
## para un solo punto (fila 8: Cebolla / Mildiu (Polvillo negro)).
##
## Umbral de alerta (columna P8 del Excel):
##   ALERTA si: T media 6-22 C  y  HR >= 90%  y  >= 3 dias nublados/lluviosos.
##
## Indicador propuesto (columna R8):
##   Indice de humedad nocturna = coincidencia de T media 6-22 C, HR >= 90% y >= 3 dias nublados/lluviosos.
##
## La formula de esa hoja (celda AC8) suma, con sus pesos:
##     (T media entre 6 y 22 C) * 1
##     (HR media >= 90%) * 2
##     (dias lluvia/nublados >= 3) * 1
##   ... y se divide entre la suma de los pesos (4).
##
## y el semaforo de alerta (misma logica que la celda S8) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de cebolla, o sin dato climatico,
...
## ----------------------------
## La columna I8 del Excel describe la precipitacion favorable como "≥ 40 mm/10
## dias"; se usa un periodo de 10 dias para mantener esa misma ventana de
## referencia, aunque la formula de alerta en si no use la precipitacion
## directamente (usa dias de lluvia/nublados, columna X).
...
# Esta plaga/enfermedad no usa racha de dias secos en su formula, pero se
# deja el parametro por consistencia con la estructura de los demas scripts.
dias_buffer    <- 0
...
# --- 1.3 Shapefiles: region productora de cebolla y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/CEBOLLA/CEBOLLA.shp"
...
# unicamente dentro de la region de cebolla).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_cebolla_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC8 del
##     Excel) para Cebolla / Mildiu (Polvillo negro).
## ----------------------------------------------------------------------
calcular_indice_cebolla <- function(t_media_periodo, hr_media_periodo, dias_lluvia_periodo) {

  cond_t <- t_media_periodo >= 6 & t_media_periodo <= 22   # T media entre 6 y 22 C
  cond_hr <- hr_media_periodo >= 90   # HR media >= 90%
  cond_lluvia <- dias_lluvia_periodo >= 3   # dias lluvia/nublados >= 3

  # pesos: 1, 2, 1 (tal cual columnas Z8:AB8 del Excel)
  (cond_t * 1 + cond_hr * 2 + cond_lluvia * 1) / 4
}

## ----------------------------------------------------------------------
## 3.3 Clasificacion de alerta tipo semaforo (misma logica en las 15 fichas
...
# recortar y enmascarar todo a la region productora de cebolla
...
dia_lluvia     <- precip > 1                        # 1 = dia de lluvia, 0 = dia seco
...
for (i in seq_along(inicios_periodo)) {
  ...
  dias_lluvia_periodo <- sum(dia_lluvia[[idx]])                # conteo de dias de lluvia en el periodo

  indice <- calcular_indice_cebolla(t_media_periodo, hr_media_periodo, dias_lluvia_periodo)
  ...
  cond_t_r <- t_media_periodo >= 6 & t_media_periodo <= 22
  cond_hr_r <- hr_media_periodo >= 90
  cond_lluvia_r <- dias_lluvia_periodo >= 3
  ...
  fila$pct_area_cond_lluvia <- round(100 * as.numeric(global(cond_lluvia_r, "mean", na.rm = TRUE)), 1)
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Mildiu (Polvillo negro) en Cebolla",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 6-22C, HR >= 90% y >=3 dias lluvia/nublados") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_cebolla_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_cebolla_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_cebolla_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_cebolla_2025.csv"),
          row.names = FALSE)

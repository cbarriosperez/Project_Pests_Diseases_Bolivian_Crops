## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - LECHUGA / SCLEROTINIA SCLEROTIORUM
...
## para un solo punto (fila 12: Lechuga / Sclerotinia sclerotiorum).
##
## Umbral de alerta (columna P12 del Excel):
##   ALERTA si: T media 15-20 C  y  HR >= 85%  y  precipitacion >= 60 mm.
##
## Indicador propuesto (columna R12):
##   Indice de saturacion = coincidencia de T media 15-20 C, HR >= 85% y precipitacion >= 60 mm (suelo saturado).
##
## La formula de esa hoja (celda AC12) suma, con sus pesos:
##     (T media entre 15 y 20 C) * 1
##     (HR media >= 85%) * 1
##     (precipitacion acumulada >= 60 mm) * 2
##   ... y se divide entre la suma de los pesos (4).
##
## y el semaforo de alerta (misma logica que la celda S12) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de lechuga, o sin dato climatico,
...
## ----------------------------
## La columna I12 define la precipitacion favorable como "≥ 60 mm/15 dias"; se
## usa un periodo de 15 dias para que la acumulacion de precipitacion sea
## comparable con esa referencia.
## Por eso PERIODO_DIAS = 15 es el valor recomendado por defecto.
...
# --- 1.3 Shapefiles: region productora de lechuga y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/LECHUGA/LECHUGA.shp"
...
# unicamente dentro de la region de lechuga).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_lechuga_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC12 del
##     Excel) para Lechuga / Sclerotinia sclerotiorum.
## ----------------------------------------------------------------------
calcular_indice_lechuga <- function(t_media_periodo, hr_media_periodo, precip_acum_periodo) {

  cond_t <- t_media_periodo >= 15 & t_media_periodo <= 20   # T media entre 15 y 20 C
  cond_hr <- hr_media_periodo >= 85   # HR media >= 85%
  cond_precip <- precip_acum_periodo >= 60   # precipitacion acumulada >= 60 mm

  # pesos: 1, 1, 2 (tal cual columnas Z12:AB12 del Excel)
  (cond_t * 1 + cond_hr * 1 + cond_precip * 2) / 4
}
...
# recortar y enmascarar todo a la region productora de lechuga
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_lechuga(t_media_periodo, hr_media_periodo, precip_acum_periodo)
  ...
  cond_t_r <- t_media_periodo >= 15 & t_media_periodo <= 20
  cond_hr_r <- hr_media_periodo >= 85
  cond_precip_r <- precip_acum_periodo >= 60
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Sclerotinia sclerotiorum en Lechuga",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media 15-20C, HR >= 85% y precip. >= 60 mm") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_lechuga_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_lechuga_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_lechuga_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_lechuga_2025.csv"),
          row.names = FALSE)

## ============================================================================
## INDICE AGROCLIMATICO DE RIESGO - UVA / MILDIU DE LA UVA
...
## para un solo punto (fila 19: Uva / Mildiu de la uva).
##
## Umbral de alerta (columna P19 del Excel):
##   ALERTA si: T media >= 10 C  y  precipitacion >= 10 mm  y  HR >= 85%.
##
## Indicador propuesto (columna R19):
##   Indice de infeccion primaria = coincidencia de T media >= 10 C, precipitacion >= 10 mm y HR >= 85% (mojado foliar).
##
## La formula de esa hoja (celda AC19) suma, con sus pesos:
##     (T media >= 10 C) * 1
##     (precipitacion acumulada >= 10 mm) * 1
##     (HR media >= 85%) * 1
##   ... y se divide entre la suma de los pesos (3).
##
## y el semaforo de alerta (misma logica que la celda S19) es:
##   indice = 0    -> "Normal"
##   indice < 0.5  -> "Alerta baja"
##   indice < 1    -> "Alerta media"
##   indice = 1    -> "Alerta alta"
## (los pixeles fuera de la region de uva, o sin dato climatico,
...
##
## ============================================================================
## POR QUE PERIODOS DE 7 DIAS
## ----------------------------
## La columna I19 menciona dos ventanas ("≥10mm/24-48h" para infeccion primaria
## puntual, y "≥30mm/7 dias" como acumulado semanal); se usa un periodo de 7 dias
## por ser la ventana de acumulacion multi-dia mas relevante para el monitoreo
## periodico (el evento de 24-48h es demasiado corto para una periodizacion
## regular).
## Por eso PERIODO_DIAS = 7 es el valor recomendado por defecto.
...
# --- 1.1 Periodizacion --------------------------------------------------
periodo_dias   <- 7                       # largo de cada periodo (dias). Recomendado: 7
...
# --- 1.3 Shapefiles: region productora de uva y mapa base de municipios ---
ruta_shp_cultivo     <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/Shapefiles/UVA/UVA.shp"
...
# unicamente dentro de la region de uva).
...
carpeta_salida <- "D:/OneDrive - CGIAR/Otras Colaboraciones/Proyectos Formulados/2024/Yapu/Proyecto_SAT de Plagas y enfermedades en Bolivia/Scripts_Modelos agroclimaticos por cultivo/salidas_uva_2025"
...
## ----------------------------------------------------------------------
## 3.2 Formula EXACTA del indice de riesgo (replica celda AC19 del
##     Excel) para Uva / Mildiu de la uva.
## ----------------------------------------------------------------------
calcular_indice_uva <- function(t_media_periodo, precip_acum_periodo, hr_media_periodo) {

  cond_t <- t_media_periodo >= 10   # T media >= 10 C
  cond_precip <- precip_acum_periodo >= 10   # precipitacion acumulada >= 10 mm
  cond_hr <- hr_media_periodo >= 85   # HR media >= 85%

  # pesos: 1, 1, 1 (tal cual columnas Z19:AB19 del Excel)
  (cond_t * 1 + cond_precip * 1 + cond_hr * 1) / 3
}
...
# recortar y enmascarar todo a la region productora de uva
...
for (i in seq_along(inicios_periodo)) {
  ...
  indice <- calcular_indice_uva(t_media_periodo, precip_acum_periodo, hr_media_periodo)
  ...
  cond_t_r <- t_media_periodo >= 10
  cond_precip_r <- precip_acum_periodo >= 10
  cond_hr_r <- hr_media_periodo >= 85
  ...
  mapa <- ggplot() +
    ...
    labs(title = "Riesgo de Mildiu de la uva en Uva",
         subtitle = paste0("Periodo: ", format(ini, "%d-%b-%Y"), " a ", format(fin, "%d-%b-%Y")),
         caption = "Indice = coincidencia de T media >=10C, precip. >=10mm y HR >=85%") +
    theme_minimal()
  ...
  ggsave(file.path(carpeta_salida,
                    paste0("mapa_alerta_uva_", format(ini, "%Y%m%d"), "_", format(fin, "%Y%m%d"), ".png")),
...
writeRaster(rast(resultados),        file.path(carpeta_salida, "indice_riesgo_uva_2025.tif"), overwrite = TRUE)
writeRaster(rast(resultados_alerta), file.path(carpeta_salida, "estado_alerta_uva_2025.tif"),  overwrite = TRUE)

write.csv(resumen_tabla, file.path(carpeta_salida, "resumen_superficie_alerta_uva_2025.csv"),
          row.names = FALSE)
